package app.rk.food.database.client.query;
...
import app.rk.food.database.DBHandler;
import app.rk.food.database.client.table.CartTable;
import app.rk.food.model.FoodData;
import app.rk.food.utils.DateTimeUtils;
 ...
 */
public class CartQuery extends DBHandler {

    public static String TABLE=CartTable.CART_TABLE;
    ...
    public int isProductInCart(String productID) {
        String _SQLiteQuery = "SELECT  * FROM " + TABLE+" WHERE "+CartTable.PRODUCT_ID+" = \""+productID+"\"";
        Cursor cursor = database.rawQuery(_SQLiteQuery, null);
        int count=cursor.getCount();
        cursor.close();
        return count;
    }
    ...
    public FoodData getCartDetailsByID(String productId){
        String selectQuery = "SELECT  * FROM " + TABLE +" WHERE "+CartTable.PRODUCT_ID+" = "+"\""+productId+"\"";
        FoodData foodData = new FoodData();
        Cursor cursor = database.rawQuery(selectQuery, null);
        try {
            if (cursor.moveToFirst()) {
